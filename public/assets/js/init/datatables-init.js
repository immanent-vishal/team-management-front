$(document).ready(function () {
    ...
    const sorting = document.querySelectorAll('.selectpicker');
    const commentSorting = document.querySelector('.selectpicker');
    for (const sr of sorting) {
        const sortingchoices = new Choices(sr, {
            placeholder: false,
            itemSelectText: ''
        });

        // Trick to apply your custom classes to generated dropdown menu
        let sortingClass = sr.getAttribute('class');
        window.onload = function () {
            sr.parentElement.setAttribute('class', sortingClass);
        }
    }
