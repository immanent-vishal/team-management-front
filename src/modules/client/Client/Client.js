...
const Client = () => {
  ...
  const remove = (e, data) => {
    ...
    if (updatedItemId.length == 1) {
      if (r) {
        ...
        clientHTTPService.removeClient(updatedItemId).then((data) => {
          resfresh();
        });
      }
    } else if (data.length == clients.length) {
      clientHTTPService.removeAllClient(updatedItemId).then((data) => {
        resfresh();
      });
    } else {
      console.log("delete hone ja rha h");

      clientHTTPService.removeSelectClient(updatedItemId).then((data) => {
        resfresh();
      });
    }
  };
  ...
  console.log("aaayi", updatedItemId);

  return (
    <div className="card">
      <div className="card-header">
        <h4>
          <i class="menu-icon fa fa-handshake-o"></i> Clients
        </h4>
      </div>
