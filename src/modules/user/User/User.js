...
const User = () => {
  ...
  const remove = (e, data) => {
    console.log("uuff" , updatedItemId.length==1);
    e.preventDefault();
    var r = window.confirm(CurrentUser.DELTE_MSG);
    console.log(updatedItemId , "1234");
  ...
  }else if(data.length==users.length){
    userHTTPService.removeAllUser(updatedItemId).then(data => {
      resfresh();
    }) 
   }
   else{
    ...
    userHTTPService.removeSelectUser(updatedItemId).then(data => {
      resfresh();
    }) 
  ...
  const columns = [
    ...
    { field: 'telephone', headerName: 'Phone', width: 200 }
  ];
  ...
  console.log(updatedItemId , "12345");
  return (
    <div className="card">
